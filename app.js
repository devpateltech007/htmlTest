...
function ShowVideoCallElements() {
    document.getElementById("actions").style.display = "block";
}

let userNameFirebase = 'Joe';
...
function load() {
    ...
    addResponseEventListener('logListener', (response) => {
        joinedMeeting = true;
        userNameFirebase = response;
        ShowVideoCallElements();
        console.log(`Received response message from streamer: "${response}"`);
        console.log(`Joined Meeting VARIABLE "${joinedMeeting}"`)
        // Open a session for the user
        // VoxeetSDK.session.open({ name: 'Sound' });
