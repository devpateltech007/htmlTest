...
const initUI = () => {
  ...
  const intervalSet = setInterval(checkUpdate, 1000);
...
const addVideoNode = (participant, stream) => {
  ...
  if (!videoNode) {
    ...
    videoNode.setAttribute('id', 'videoPlayer');
...
const addParticipantNode = (participant) => {
  ...
  if (participant.id === VoxeetSDK.session.participant.id) return;
  if (participant.name === "Sound") return;
...
const addScreenShareNode = (stream) => {
  ...
  screenShareNode.setAttribute('height', 240);
  screenShareNode.setAttribute('width', 320);
  screenShareNode.setAttribute("playsinline", true);
...
const removeScreenShareNode = () => {
  let screenShareNode = document.getElementById('screenshare');
  let screenShareContainer = document.getElementById('screenshare-container');
